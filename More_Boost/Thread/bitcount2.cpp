...
unsigned long bitCount_mt(const unsigned char byte[], size_t size,
	...
	const size_t psize= size / nthreads;
	ptr_vector<BitCounter> workers;
	ptr_vector<packaged_task<unsigned long> > tasks;
	ptr_vector<unique_future<unsigned long> > futures;
	for (int i= 0; i < nthreads; ++i) {
		const size_t nbytes= (i < nthreads-1)
					? psize
					: (size - (nthreads-1)*psize);
		workers.push_back(new BitCounter(&byte[i*psize], nbytes, f));
		tasks.push_back(new packaged_task<unsigned long>(workers.at(i)));
		futures.push_back(new unique_future<unsigned long>(tasks.at(i).get_future()));
		thread(move(tasks.at(i)));
	}
	unsigned long count= 0;
	for (int i= 0; i < nthreads; ++i)
		count += futures.at(i).get();
	return count;
}
